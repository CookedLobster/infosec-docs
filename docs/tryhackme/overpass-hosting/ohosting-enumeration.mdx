...
Inside the **Backup.zip File** we have `priv.key` - `CustomerDetails.xlsx.gpg`. We Import the `priv.key` && Decrypt `CustomerDetails.xlsx.gpg`
...
```
  ...
  <TabItem value="decrypt" label="DECRYPT">
    <div style={{color: 'red'}}>
